...
const Navbar = ({setShowLogin}) => {
  ...
  return (
    ...
    className={`flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4
     text-gray-600 border-b borderColor relative transition-all 
     ${location.pathname === '/' && "bg-light"}`}>
        ...
        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 hidden sm:flex
        max-sm:border-t border-borderColor right-0  flex-col sm:flex-row 
        ...
        {open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
            ...
            px-3 rounded-full max-w-56'>
                   <input type="text" className="py-1.5 w-full bg-transparent outline-none placeholder-gray-500" placeholder='Search prouduct' />
